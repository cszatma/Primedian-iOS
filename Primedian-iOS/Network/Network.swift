...
func fetchResult(upperLimit: UInt64, session: NetworkSession = URLSession.shared, completion: @escaping (Status) -> Void) {
    ...
    session.fetch(from: url) { result in
        ...
        case .success(let data, let response):
            guard let res = response as? HTTPURLResponse, res.mimeType == "application/json" else {
        ...
        case .failure(let error):
            completion(.error(.unknown(message: error.localizedDescription)))
        }
    }
}
